...
#version used 256 max token length, 
#since this version i optimized the prompt length and specified
#QA length and accounted for token length to try to get good responses
#that arent cut off
...
def generate_dialogue(subject):
    prompt = f""role: character from ancient fantasy rpg. Write brief question and answer about {subject}. question is one sentence 15 words or less and answer is one sentence 35 words or less. Enclose the question in *1* <question> *1* and the answer in *2* <answer> *2*.""
"
    generated_text = text_generator(prompt, max_length=160, truncation=True)[0]['generated_text']
